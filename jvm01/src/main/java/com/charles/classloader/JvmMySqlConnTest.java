...
/**
 * @author chales.tang
 * @title JvmMySqlConnTest
 * @discription Mysql驱动包下Driver驱动类加载测试
 */
public class JvmMySqlConnTest {

    public static void main(String[] args) throws SQLException, ClassNotFoundException {

        // 设置当前线程的类加载器为 "扩展类加载器" ，会导致线程加载不到mysql驱动包下java.sql.Driver的实现类
        Thread.currentThread().setContextClassLoader(JvmMySqlConnTest.class.getClassLoader().getParent());

        Connection root = DriverManager.getConnection(
                "jdbc:mysql://127.0.0.1:3306/mysql?characterEncoding=UTF-8",
                "root", "root");

        // DriverManager 通过SPI机制懒加载mysql驱动包下java.sql.Driver的实现类
